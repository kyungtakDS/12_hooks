...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌 실제 실행 가능한 검증 커맨드를 포함한다. 커맨드는 **반드시 CLAUDE.md의 `## 명령어` 절에 있는 것만** 쓴다. 없는 커맨드를 지어내면 AC가 매번 실패한다. (이 저장소 기준: `python -m pytest scripts/ -q`)
...
{CLAUDE.md 의 `## 명령어` 절에 있는 커맨드만 쓴다. 이 저장소는 아래가 전부다 —
lint·build 단계가 없으므로 넣지 마라. 실행되지 않는다.}

```bash
python -m pytest scripts/ -q   # 테스트 통과
```
