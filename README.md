...
| 상황 | 동작 |
|---|---|
| **시크릿·자격증명 발견** | **push 차단** (exit 1), 파일:라인과 이유 출력 |
| 그 외 문제 발견 | 경고 출력 후 **통과** (exit 0) |
| 문제 없음 | 통과 |
...
**왜 시크릿만 막는가** — 측정된 정확도가 영역별로 갈렸다. 시크릿 검출은 2/2 정확했지만
그 외 지적은 4건 중 3건이 오탐이었다(존재하는 액션 버전을 없다고 함, 이미 있는 `.gitattributes`
설정이 빠졌다고 함, 인용된 변수가 단어 분할된다고 함). 차단형 게이트가 이 비율이면
`--no-verify`가 습관이 되어 **시크릿조차 막지 못하게 된다.** 자세한 근거는 ADR-011.
